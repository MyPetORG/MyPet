...
public class Poison extends PoisonInfo implements ISkillInstance, ISkillActive
{
    ...
    public void upgrade(ISkillInfo upgrade, boolean quiet)
    {
        if (upgrade instanceof PoisonInfo)
        {
            ...
            if (!quiet && valuesEdit)
            {
                myPet.sendMessageToOwner(MyPetBukkitUtil.setColors(MyPetLanguage.getString("Msg_PoisonChance")).replace("%petname%", myPet.petName).replace("%chance%", "" + chance).replace("%duration%", "" + duration));
            }
        }
    }

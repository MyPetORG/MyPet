...
public class Wither extends MyPetGenericSkill
{
    ...
    public void upgrade(MyPetSkillTreeSkill upgrade, boolean quiet)
    {
        if (upgrade instanceof Wither)
        {
            ...
            if (!quiet && valuesEdit)
            {
                myPet.sendMessageToOwner(MyPetUtil.setColors(MyPetLanguage.getString("Msg_WitherChance")).replace("%petname%", myPet.petName).replace("%chance%", "" + chance).replace("%duration%", "" + duration));
            }
        }
    }
    ...
    {
        MyPetSkillTreeSkill newSkill = new Wither(this.isAddedByInheritance());

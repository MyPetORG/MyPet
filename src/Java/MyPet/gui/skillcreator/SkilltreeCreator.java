...
package Java.MyPet.gui.skillcreator;

import Java.MyPet.gui.skillcreator.MyPetSkillTreeConfig.MyPetSkillTree;
        ...
        catch (ClassNotFoundException e)
        {
            ...
            else if (result == 2)
            {
                ...
                if (fileChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)
                {
                    try
                    {
                        Runtime.getRuntime().exec("java -cp \"" + fileChooser.getSelectedFile().getAbsolutePath() + "\"" + System.getProperties().getProperty("path.separator") + "\"" + (SkilltreeCreator.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath()) + "\" SkilltreeCreator");
                    }
                    catch (Exception e1)
                    {
                        e1.printStackTrace();

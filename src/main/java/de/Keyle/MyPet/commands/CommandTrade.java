...
public class CommandTrade implements CommandExecutor, TabCompleter {
            ...
            if (args[0].equalsIgnoreCase("accept")) {
                if (offers.containsKey(player.getUniqueId())) {
                    ...
                    if (PlayerList.isMyPetPlayer(owner)) {
                        final MyPetPlayer oldOwner = PlayerList.getMyPetPlayer(owner);
                        if (!oldOwner.hasMyPet() || oldOwner.getMyPet() != offer.getPet()) {
                            sender.sendMessage(Translation.getString("Message.Command.Trade.Reciever.PetUnavailable", player));
                            offers.remove(player.getUniqueId());
                            return true;
                        }
                        if (PlayerList.isMyPetPlayer(player) && MyPetList.hasActiveMyPet(player)) {
                        ...
                        MyPetPlugin.getPlugin().getRepository().getMyPet(offer.getPet().getUUID(), new RepositoryCallback<InactiveMyPet>() {
                            ...
                            public void callback(InactiveMyPet pet) {
                                ...
                                if (myPet != null) {
                                    ...
                                    myPet.sendMessageToOwner(Translation.getString("Message.Command.Trade.Reciever.Error", newOwner));
                                }
                            }
                        });
                    } else {
                ...
                if (MyPetList.hasActiveMyPet(player)) {
                    ...
                    FancyMessage petMessage = new FancyMessage(" »» ")
                            ...
                            .itemTooltip(Util.myPetToItemTooltip(myPet, BukkitUtil.getPlayerLanguage(reciever)))
                            .command("/pettrade accept");
                    BukkitUtil.sendMessageRaw(reciever, petMessage.toJSONString());
                    return true;
                } else {
    ...
    private class Offer {
        ...
        public Offer(double price, MyPet pet, UUID owner, UUID reciever, String recieverName) {
            this.price = price;
            this.pet = pet;
            this.owner = owner;

...
public class Wither extends WitherInfo implements ISkillInstance, ISkillActive
{
    ...
    public void upgrade(ISkillInfo upgrade, boolean quiet)
    {
        if (upgrade instanceof WitherInfo)
        {
            ...
            if (!quiet && valuesEdit)
            {
                myPet.sendMessageToOwner(MyPetBukkitUtil.setColors(MyPetLanguage.getString("Msg_WitherChance")).replace("%petname%", myPet.petName).replace("%chance%", "" + chance).replace("%duration%", "" + duration));
            }
        }
    }

...
public class Wither extends WitherInfo implements ISkillInstance, ISkillActive {
    ...
    public void upgrade(ISkillInfo upgrade, boolean quiet) {
        if (upgrade instanceof WitherInfo) {
            ...
            if (!quiet && valuesEdit) {
                myPet.sendMessageToOwner(Util.formatText(Translation.getString("Message.Skill.Wither.Upgrade", myPet.getOwner().getLanguage()), myPet.getPetName(), chance, duration));
            }
        }
    }

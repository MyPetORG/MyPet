...
public class PathfinderGoalAggressive extends PathfinderGoal
{
                ...
                else if (behavior.getBehavior() == Behavior.BehaviorState.Aggressive && !MWolf.isSitting())
                {
                    if (target == null || !target.isAlive())
                    {
                        ...
                        for (Object aList : list)
                        {
                            ...
                            if (wolf.al().canSee(entityliving) && !(entityliving instanceof EntityHuman && ((EntityHuman)entityliving).name.equals(MWolf.getOwner().getName())))
                            {
    ...
    public void c()
    {
        wolf.ak().a(target.getBukkitEntity().getLocation().getX(), target.getBukkitEntity().getLocation().getY(), target.getBukkitEntity().getLocation().getZ(), 0.5f);

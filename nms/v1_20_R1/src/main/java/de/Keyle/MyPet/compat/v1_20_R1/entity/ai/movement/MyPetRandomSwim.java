...
public class MyPetRandomSwim extends MyPetRandomStroll {
    ...
    protected Vec3 getPosition() {
        boolean petInWater = petEntity.isInWater() || petEntity.getBlockStateOn().is(Blocks.BUBBLE_COLUMN);
        boolean ownerInWater = owner.isInWater() || owner.getBlockStateOn().is(Blocks.BUBBLE_COLUMN);
    ...
    private Vec3 makeWaterPos(PathfinderMob mob, int horizontalRadius, int verticalRadius) {
        ...
        for (int attempt = 0; attempt < WATER_POS_MAX_ITERATIONS; attempt++) {
            ...
            boolean isWaterPathfindable = mob.level().getBlockState(blockPos).isPathfindable(mob.level(), blockPos, PathComputationType.WATER);

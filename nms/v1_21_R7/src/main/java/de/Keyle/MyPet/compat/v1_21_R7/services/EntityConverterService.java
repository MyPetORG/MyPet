    ...
    public void convertEntity(MyPet myPet, LivingEntity normalEntity) {
        if (myPet instanceof MyCreeper creeper) {
            ...
            nmsEntity.setVillagerData(nmsEntity.getVillagerData()
                    .withType(VILLAGER_TYPE_REGISTRY.wrapAsHolder(VILLAGER_TYPE_REGISTRY.getValue(Identifier.tryParse(zombieVillager.getType().name().toLowerCase(Locale.ROOT)))))
            ...
            try {
                ...
                if (copperGolem.hasPoppy()) {
                    ItemStack bukkitPoppy = copperGolem.getPoppy();

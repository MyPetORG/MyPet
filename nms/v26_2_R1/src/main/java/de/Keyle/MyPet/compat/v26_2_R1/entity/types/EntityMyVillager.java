...
import de.Keyle.MyPet.compat.v26_2_R1.services.EntityConverterService;
...
public class EntityMyVillager extends EntityMyPet {
	...
	public InteractionResult handlePlayerInteraction(Player entityhuman, InteractionHand enumhand, ItemStack itemStack) {
		...
		if (getOwner().equals(entityhuman) && itemStack != null && canUseItem() && getOwner().getPlayer().isSneaking()) {
			...
			if (itemStack.getItem() != Items.SHEARS && canEquip()) {
				...
				getMyPet().setEquipment(EquipmentSlot.MainHand, CompatItemStack.asBukkitCopy(itemStack));
				if (itemStack != ItemStack.EMPTY && !entityhuman.getAbilities().instabuild) {
					itemStack.shrink(1);
					if (itemStack.getCount() <= 0) {
						entityhuman.getInventory().setItem(entityhuman.getInventory().getSelectedSlot(), ItemStack.EMPTY);
	...
	public void updateVisuals() {
		...
		VillagerProfession profession = BuiltInRegistries.VILLAGER_PROFESSION.get(Identifier.tryParse(professionKey)).get().value();
		VillagerType type = BuiltInRegistries.VILLAGER_TYPE.get(Identifier.tryParse(getMyPet().getType().getKey())).get().value(); //TODO
	...
	public void setPetEquipment(ItemStack itemStack) {
		((ServerLevel) this.level()).getChunkSource().sendToTrackingPlayersAndSelf(this, new ClientboundSetEquipmentPacket(getId(), List.of(new Pair<>(net.minecraft.world.entity.EquipmentSlot.MAINHAND, itemStack))));
	}

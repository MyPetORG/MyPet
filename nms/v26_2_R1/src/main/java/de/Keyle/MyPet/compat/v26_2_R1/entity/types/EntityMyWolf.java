...
public class EntityMyWolf extends EntityMyPet {
	...
	private static final EntityDataAccessor<Boolean> AGE_LOCKED_WATCHER = SynchedEntityData.defineId(EntityMyWolf.class, EntityDataSerializers.BOOLEAN);
		...
		if (getOwner().equals(entityhuman)) {
			if (itemStack != null && itemStack.getItem() != Items.AIR) {
				if (canUseItem()) {
					...
					if (nmsDyeColor != null && nmsDyeColor.ordinal() != getMyPet().getCollarColor().ordinal()) {
						if (getOwner().getPlayer().isSneaking()) {
							getMyPet().setCollarColor(DyeColor.values()[nmsDyeColor.ordinal()]);
	...
	public void updateVisuals() {
		...
		if (varRegistry == null)
			this.varRegistry = this.registryAccess().lookupOrThrow(Registries.WOLF_VARIANT);
		Optional<Holder.Reference<WolfVariant>> variantOptional = varRegistry.get(Identifier.tryParse(getMyPet().getVariant()));

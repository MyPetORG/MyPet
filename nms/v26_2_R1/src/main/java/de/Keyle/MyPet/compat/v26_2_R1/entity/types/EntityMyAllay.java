...
import de.Keyle.MyPet.compat.v26_2_R1.entity.EntityMyFlyingPet;
		...
		if (getOwner().equals(entityhuman)) {
			if (itemStack != null && itemStack.getItem() != Items.AIR && canUseItem() && getOwner().getPlayer().isSneaking()) {
				...
				if (itemStack.getItem() != Items.SHEARS && getOwner().getPlayer().isSneaking() && canEquip()) {
					...
					if (hadEquipment) {
						this.playSound(BuiltInRegistries.SOUND_EVENT.get(Identifier.tryParse("entity.allay.item_thrown")).get().value());
						if (itemStack != ItemStack.EMPTY && !entityhuman.getAbilities().instabuild) {
							try {

...
public class EntityMyCat extends EntityMyPet {
	...
	public InteractionResult handlePlayerInteraction(Player entityhuman, InteractionHand enumhand, ItemStack itemStack) {
		...
		if (getOwner().equals(entityhuman)) {
			if (itemStack != null && canUseItem() && getOwner().getPlayer().isSneaking()) {
				net.minecraft.world.item.DyeColor nmsDyeColor = itemStack.get(DataComponents.DYE);
				if (nmsDyeColor != null) {
					if (nmsDyeColor.getId() != getMyPet().getCollarColor().ordinal()) {
						getMyPet().setCollarColor(DyeColor.values()[nmsDyeColor.getId()]);
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(VARIANT_WATCHER, registry.wrapAsHolder(VariantConverter.CAT_REGISTRY.getOrThrow(CatVariants.BLACK).value()));
	...
	public void updateVisuals() {
		...
		Registry<CatVariant> registry = CraftRegistry.getMinecraftRegistry(Registries.CAT_VARIANT);
		this.getEntityData().set(VARIANT_WATCHER, registry.wrapAsHolder(VariantConverter.convertCatVariant(MyCat.getOwnTypeOrdinal(getMyPet().getCatType()))));

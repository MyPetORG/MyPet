...
public class EntityMyWarden extends EntityMyPet {
	...
	public void tick() {
		...
		if(this.heartAttack) {
			this.playSound(BuiltInRegistries.SOUND_EVENT.get(Identifier.tryParse("entity.warden.heartbeat")).get().value());
		}
	}
	...
	private void makeSound(String leSound) {
		SoundEvent se = BuiltInRegistries.SOUND_EVENT.get(Identifier.tryParse(leSound)).get().value();

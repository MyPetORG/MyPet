...
public class EntityConverterService extends de.Keyle.MyPet.api.util.service.types.EntityConverterService {
    ...
    public void convertEntity(MyPet myPet, LivingEntity normalEntity) {
        if (myPet instanceof MyCreeper creeper) {
            ...
            VillagerData villagerData = new VillagerData(type, profession, villagerPet.getLevel());
            ...
            if (villagerPet.hasOriginalData()) {
                ...
                try {
                    ...
                    if (villagerTag.keySet().contains("FoodLevel")) {
                        ...
                        ReflectionUtil.setFieldValue("cr", entityVillager, foodLevel);        // Field: foodLevel
                    }
                    ...
                    if (villagerTag.keySet().contains("LastRestock")) {
                        ...
                        ReflectionUtil.setFieldValue("cw", entityVillager, lastRestock);    //Field: lastRestock(Game)Time
                    }
                    if (villagerTag.keySet().contains("LastGossipDecay")) {
                        long lastGossipDecay = villagerTag.getLong("LastGossipDecay");
                        ReflectionUtil.setFieldValue("cu", entityVillager, lastGossipDecay);    //Field: lastGossipDecayTime
                    }
                    if (villagerTag.keySet().contains("RestocksToday")) {
                        int restocksToday = villagerTag.getInt("RestocksToday");
                        ReflectionUtil.setFieldValue("cx", entityVillager, restocksToday);        //Field: numberOfRestocksToday or restocksToday
                    }
                    ReflectionUtil.setFieldValue("cz", entityVillager, true); // Field: AssignProfessionWhenSpawned (natural?)
                } catch (Exception e) {
                    ErrorUtil.report(e);
                }
            ...
            nmsEntity.setVillagerData(nmsEntity.getVillagerData()
                    .setType(BuiltInRegistries.VILLAGER_TYPE.get(ResourceLocation.tryParse(zombieVillager.getType().name().toLowerCase(Locale.ROOT))).get().value())
        ...
        } else if (myPet instanceof MyCamel camel) {
            if (camel.hasSaddle()) {
                ((Camel) normalEntity).getInventory().setSaddle(camel.getSaddle().clone());
            }
        ...
        } else if (myPet instanceof MySalmon salmon) {
            ((Salmon) normalEntity).setVariant(Salmon.Variant.values()[salmon.getVariant()]);
            ...
            if (traderPet.hasOriginalData()) {
                ...
                for (String key : villagerTag.keySet()) {
                    builder.put(key, villagerTag.get(key));
                }
                // Add/override with original data
                for (String key : traderPet.getOriginalData().keySet()) {
                    builder.put(key, traderPet.getOriginalData().get(key));
                }
                MyPetApi.getPlatformHelper().applyTagToEntity(builder.build(), normalEntity);
            }
    ...
    public void convertCamel(Camel camel, CompoundBinaryTag.Builder builder) {
        // Write saddle using unified Equipment list format
        if (camel.getInventory().getSaddle() != null && camel.getInventory().getSaddle().getType() != Material.AIR) {
            List<CompoundBinaryTag> equipmentList = new ArrayList<>();

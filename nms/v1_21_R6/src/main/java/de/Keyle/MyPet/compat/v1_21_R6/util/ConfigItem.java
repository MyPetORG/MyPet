...
import de.Keyle.MyPet.compat.v1_21_R6.util.inventory.ItemStackComparator;
...
public class ConfigItem extends de.Keyle.MyPet.api.util.ConfigItem {
    ...
    public void load(String materialId, String data) {
        ResourceLocation key = ResourceLocation.tryParse(materialId);
        ...
        if (data != null) {
            ...
            if (nbtString.startsWith("{") && nbtString.endsWith("}")) {
                try {
                    CompoundTag nbtData = TagParser.parseCompoundFully(nbtString);

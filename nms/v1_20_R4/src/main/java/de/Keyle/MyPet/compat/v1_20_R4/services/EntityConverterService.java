...
public class EntityConverterService extends de.Keyle.MyPet.api.util.service.types.EntityConverterService {
    ...
    public TagCompound convertEntity(LivingEntity entity) {
        ...
        switch (entity.getType()) {
            ...
            case DONKEY:
                ...
                break;
            case CAMEL:
                convertCamel((Camel) entity, properties);
                break;
    ...
    public void convertEntity(MyPet myPet, LivingEntity normalEntity) {
        if (myPet instanceof MyCreeper) {
            ...
            if (villagerPet.hasOriginalData()) {
                ...
                try {
                    ...
                    if (villagerTag.containsKey("FoodLevel")) {
                        ...
                        ReflectionUtil.setFieldValue("cx", entityVillager, foodLevel);        // Field: foodLevel
                    }
                    if (villagerTag.containsKey("Gossips")) {
                        ...
                        //((GossipContainer) ReflectionUtil.getFieldValue(net.minecraft.world.entity.npc.Villager.class, entityVillager, "cy")) //Field: gossips
                    ...
                    if (villagerTag.containsKey("LastRestock")) {
                        ...
                        ReflectionUtil.setFieldValue("cD", entityVillager, lastRestock);    //Field: lastRestock(Game)Time
                    }
                    if (villagerTag.containsKey("LastGossipDecay")) {
                        ...
                        ReflectionUtil.setFieldValue("cA", entityVillager, lastGossipDecay);    //Field: lastGossipDecayTime
                    }
                    if (villagerTag.containsKey("RestocksToday")) {
                        int restocksToday = villagerTag.getAs("RestocksToday", TagInt.class).getIntData();
                        ReflectionUtil.setFieldValue("cE", entityVillager, restocksToday);        //Field: numberOfRestocksToday or restocksToday
                    }
                    ReflectionUtil.setFieldValue("cG", entityVillager, true); // Field: AssignProfessionWhenSpawned (natural?)
                } catch (Exception e) {
                    ErrorUtil.report(e);
                }
            ...
            nmsEntity.setVillagerData(nmsEntity.getVillagerData()
                    .setType(BuiltInRegistries.VILLAGER_TYPE.get(new ResourceLocation(((MyZombieVillager) myPet).getType().name().toLowerCase(Locale.ROOT))))
    ...
    public void convertHorse(Horse horse, TagCompound properties) {
        ...
        if (horse.getInventory().getArmor() != null && horse.getInventory().getArmor().getType() != Material.AIR) {
            ...
            armor.getCompoundData().put("Slot", new TagString(EquipmentSlot.BODY.name()));
    ...
    public void convertCamel(Camel camel, TagCompound properties) {
        // Write saddle using unified Equipment list format
        if (camel.getInventory().getSaddle() != null && camel.getInventory().getSaddle().getType() != Material.AIR) {
            List<TagCompound> equipmentList = new ArrayList<>();
            TagCompound saddle = MyPetApi.getPlatformHelper().itemStackToCompund(camel.getInventory().getSaddle());
            saddle.getCompoundData().put("Slot", new TagString("SADDLE"));
            equipmentList.add(saddle);
            properties.getCompoundData().put("Equipment", new TagList(equipmentList));
        }

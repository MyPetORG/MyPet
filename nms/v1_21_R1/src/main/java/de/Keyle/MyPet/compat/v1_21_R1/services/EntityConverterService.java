    ...
    public void convertEntity(MyPet myPet, LivingEntity normalEntity) {
        if (myPet instanceof MyCreeper) {
            ...
            nmsEntity.setVillagerData(nmsEntity.getVillagerData()
                    .setType(BuiltInRegistries.VILLAGER_TYPE.get(ResourceLocation.tryParse(((MyZombieVillager) myPet).getType().name().toLowerCase(Locale.ROOT))))
                    .setLevel(((MyZombieVillager) myPet).getTradingLevel())
                    .setProfession(BuiltInRegistries.VILLAGER_PROFESSION.get(ResourceLocation.tryParse(profession.name().toLowerCase(Locale.ROOT)))));

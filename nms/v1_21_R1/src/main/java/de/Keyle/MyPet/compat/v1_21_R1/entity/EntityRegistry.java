...
public class EntityRegistry extends de.Keyle.MyPet.api.entity.EntityRegistry {
    ...
    protected void registerEntityType(MyPetType petType, String key, DefaultedRegistry<EntityType<?>> entityRegistry) {
        EntityDimensions size = entityRegistry.get(ResourceLocation.tryParse(key.toLowerCase())).getDimensions();
        EntityType<?> leType;
        ...
        EntityType<? extends LivingEntity> types = (EntityType<? extends LivingEntity>) entityRegistry.get(ResourceLocation.tryParse(key));
    ...
    protected void registerEntity(MyPetType type, DefaultedRegistry<EntityType<?>> entityRegistry) {
        Class<? extends EntityMyPet> entityClass = ReflectionUtil.getClass("de.Keyle.MyPet.compat.v1_21_R1.entity.types.EntityMy" + type.name()).asSubclass(EntityMyPet.class);
    ...
    protected int getEntityTypeId(MyPetType type, DefaultedRegistry<EntityType<?>> entityRegistry) {
        EntityType<?> types = entityRegistry.get(ResourceLocation.tryParse(type.getTypeID().toString()));

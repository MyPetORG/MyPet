...
public class Wither extends WitherInfo implements SkillInstance, ActiveSkill {
    ...
    public void upgrade(SkillInfo upgrade, boolean quiet) {
        if (upgrade instanceof WitherInfo) {
            ...
            if (!quiet && valuesEdit) {
                myPet.getOwner().sendMessage(Util.formatText(Translation.getString("Message.Skill.Wither.Upgrade", myPet.getOwner().getLanguage()), myPet.getPetName(), chance, duration));
            }
        }
    }

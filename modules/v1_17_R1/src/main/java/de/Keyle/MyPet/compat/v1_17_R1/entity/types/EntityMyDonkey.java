...
public class EntityMyDonkey extends EntityMyPet implements IJumpable {
	...
	protected static final EntityDataAccessor<Optional<UUID>> OWNER_WATCHER = SynchedEntityData.defineId(EntityMyDonkey.class, EntityDataSerializers.o);
		...
		if (!blockdata.getMaterial().isLiquid()) {
			...
			if (this.isVehicle()) {
				...
				if (this.soundCounter > 5 && this.soundCounter % 3 == 0) {
					this.playSound(SoundEffects.iB, soundeffecttype.getVolume() * 0.16F, soundeffecttype.getPitch());
				} else if (this.soundCounter <= 5) {
					this.playSound(SoundEffects.iH, soundeffecttype.getVolume() * 0.16F, soundeffecttype.getPitch());
				}
			} else if (!blockdata.getMaterial().isLiquid()) {
				this.soundCounter += 1;
				playSound(SoundEffects.iH, soundeffecttype.getVolume() * 0.16F, soundeffecttype.getPitch());
			} else {
				playSound(SoundEffects.iG, soundeffecttype.getVolume() * 0.16F, soundeffecttype.getPitch());
			}
		}
	}

...
package de.Keyle.MyPet.compat.v1_16_R3.entity.ai.attack;
...
import de.Keyle.MyPet.compat.v1_16_R3.skill.skills.ranged.nms.MyPetSmallFireball;
import de.Keyle.MyPet.compat.v1_16_R3.skill.skills.ranged.nms.MyPetSnowball;
import de.Keyle.MyPet.compat.v1_16_R3.skill.skills.ranged.nms.MyPetTrident;
...
public class RangedAttack implements AIGoal {
	...
	public boolean shouldStart() {
		...
		if (meleeDamage > 0 && this.entityMyPet.h(target.locX(), target.getBoundingBox().minY, target.locZ()) < 4) {
	...
	public boolean shouldFinish() {
		...
		double meleeDamage = myPet.getDamage();
		if (meleeDamage > 0 && this.entityMyPet.h(target.locX(), this.target.getBoundingBox().minY, target.locZ()) < 4) {
			Ranged rangedSkill = myPet.getSkills().get(Ranged.class);
			if (meleeDamage > rangedSkill.getDamage().getValue().doubleValue()) {
				return true;
			}
	...
	@Override
	public void tick() {
		double distanceToTarget = this.entityMyPet.h(this.target.locX(), this.target.getBoundingBox().minY, this.target.locZ());
	...
	public void shootProjectile(EntityLiving target, float damage, Projectile projectile) {
		...
		switch (projectile) {
			...
			case LargeFireball: {
				...
				double distanceY = minY + (double) (this.target.getHeight() / 2.0F) - (0.5D + entityMyPet.locY() + (entityMyPet.getHeight() / 2.0F));

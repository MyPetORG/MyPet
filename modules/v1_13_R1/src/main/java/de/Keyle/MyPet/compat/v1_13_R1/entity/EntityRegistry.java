...
import de.Keyle.MyPet.compat.v1_13_R1.entity.types.*;
...
public class EntityRegistry extends de.Keyle.MyPet.api.entity.EntityRegistry {
    ...
    public void registerEntityTypes() {
        ...
        if (isRegistryCustom()) {
            MyPetApi.getLogger().info("Custom entity registry found: " + EntityTypes.REGISTRY.getClass().getName());
        }
        Object[] backup = backupRegistryID(registry);

        for (MyPetType type : entityClasses.keySet()) {
            try {
                registry.a(RegistryNames.valueOf(type.name()).minecraftName, EntityTypes.a.a(entityClasses.get(type)).b().a());
    ...
    protected RegistryMaterials getRegistry() {
        if (isRegistryCustom()) {
            return getCustomRegistry(EntityTypes.REGISTRY);
        }
        return EntityTypes.REGISTRY;
    }

    protected boolean isRegistryCustom() {
        return EntityTypes.REGISTRY.getClass() != RegistryMaterials.class;
    }

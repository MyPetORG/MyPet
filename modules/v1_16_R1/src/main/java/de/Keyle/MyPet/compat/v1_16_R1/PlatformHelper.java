...
import de.Keyle.MyPet.compat.v1_16_R1.util.inventory.ItemStackNBTConverter;
...
public class PlatformHelper extends de.Keyle.MyPet.api.PlatformHelper {
    ...
    public Boolean canSpawn(Location loc, AxisAlignedBB bb) {
        net.minecraft.server.v1_16_R1.World mcWorld = ((CraftWorld) loc.getWorld()).getHandle();
    ...
    public void sendMessageRaw(Player player, String message) {
        if (player instanceof CraftPlayer) {
            player.spigot().sendMessage(net.md_5.bungee.api.ChatMessageType.CHAT, ComponentSerializer.parse(message));
        }
    }
    ...
    public Entity getEntity(int id, World world) {
        net.minecraft.server.v1_16_R1.Entity e = ((CraftWorld) world).getHandle().getEntity(id);
    ...
    public net.minecraft.server.v1_16_R1.World getWorldNMS(World world) {

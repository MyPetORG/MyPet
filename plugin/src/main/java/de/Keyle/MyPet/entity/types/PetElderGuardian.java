...
public class PetElderGuardian extends PetImpl implements PetAquaticEntity {
    ...
    /** Stops an Elder Guardian pet's vanilla spikes (thorns) from hurting its own owner. */
    public static final class SpikeOwnerProtector implements Listener {

        @EventHandler(ignoreCancelled = true)
        public void onElderGuardianSpikeDamage(EntityDamageByEntityEvent event) {
            if (event.getCause() != DamageCause.THORNS) return;
            Pet pet = PetListenerGuards.markedPet(event.getDamager()).orElse(null);
            if (!(pet instanceof PetElderGuardian)) return;

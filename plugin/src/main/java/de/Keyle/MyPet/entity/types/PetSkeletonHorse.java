...
@DefaultInfo(food = {Material.BONE}, flySpeed = 0.4405D)
public class PetSkeletonHorse extends PetImpl implements PetBaby, PetEquipment, PetNaturallyRideable, PetSaddleable, PetTameable {
    ...
    public ItemStack[] getEquipment() {
        if (!(getBukkitEntity() instanceof SkeletonHorse horse)) return new ItemStack[]{null};
        return new ItemStack[]{horse.getInventory().getSaddle()};
    }

    @Override
    public ItemStack getEquipment(EquipmentSlot slot) {
        if (!(getBukkitEntity() instanceof SkeletonHorse horse)) return null;
    ...
    protected void setEquipmentBySlotName(String slotName, ItemStack item) {
        if (!(getBukkitEntity() instanceof SkeletonHorse horse)) {
    ...
    public void dropEquipment() {
        if (status != PetState.Here || !(getBukkitEntity() instanceof SkeletonHorse horse)) return;

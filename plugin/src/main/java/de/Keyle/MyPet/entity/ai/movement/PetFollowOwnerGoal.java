 ...
 * </ul>
 */
public class PetFollowOwnerGoal implements Goal<Mob> {
     ...
     * Value of 0.5 ~ cos(60 degrees), so steering activates when Pet is
     * more than 60 degrees off-course from direct path to owner.
     */
    private static final double STEER_BASE_THRESHOLD = 0.5;

    /**
     * Maximum alignment threshold at high speeds (cosine of angle).
     * Value of 0.7 ~ cos(45 degrees). At higher speeds, steering activates
     ...
     * Must match {@link PetRandomStrollGoal#STATIONARY_MAX_DIST_SQ} to prevent
    ...
    /**
     * Sentinel for the controlPathfinderGoal lookup. Flying pets have no
    ...
    /**
     * Creates a new PetFollowOwnerGoal for the specified Pet entity.
     *
     * @param petEntity        the Pet's Bukkit entity
    ...
    public PetFollowOwnerGoal(MyPet pet, Mob mob, double startDistance, float stopDistance, float teleportDistance, boolean flyingPet, boolean aquaticPet) {
        ...
        // Owner is refreshed on each goal entry via refreshOwner() — do not
        // cache it at construction time because reconnects produce a new
        // Player instance and the old one becomes stale/offline.
        this.flyingPet = flyingPet;
        this.aquaticPet = aquaticPet;
    }

    /**
     * Refreshes the cached owner reference from the MyPetPlayer. Returns
    ...
    /**
     * Checks if the Pet is currently swimming (in water and has aquatic navigation).
     *
    ...
    private boolean isOwnerInWaterAndPetCantSwim() {
        return owner != null && owner.isInWater() && !canPetSwim() && !flyingPet;
    }
    ...
    public void stop() {
        ...
        this.nav.stop();
        // Reset speed tracking so stale data doesn't inflate the start distance
        // on the next shouldActivate() call. Without this, the goal "remembers"
        // the owner was walking and uses the full 7-block threshold even when
        // the owner has since stopped and started moving again nearby.
    ...
    public void tick() {
        ...
        // Prevents false teleports when entity position is unstable after spawn
        if (tickCounter <= 5) {
            return;
        }

        // Refresh each tick — a player that reconnects mid-tick produces a
        // new Player instance, and the previous one becomes offline/stale.
        if (!refreshOwner()) {
            return;
        }
        ...
        if (isOwnerInWaterAndPetCantSwim()) {
            ...
            double hdx = ownerLoc.getX() - petLoc.getX();
            double hdz = ownerLoc.getZ() - petLoc.getZ();
            double horizontalDistSq = hdx * hdx + hdz * hdz;
            ...
            double dirZ = hdz / horizontalDist;
            ...
            newVelX = vel.getX() * SURFACE_SWIM_BLEND + newVelX * SURFACE_SWIM_BLEND;
        ...
        if (this.pet.canMove()) {
            ...
            if (distanceSqr > FULL_STOP_DIST_SQ) {
                ...
                if (flyingPet || swimmingPet) {
                    ...
                    if (flyingPet) {
                        double targetYLevel = ownerLoc.getY() + HOVER_HEIGHT;

                        // Calculate Y offset from TARGET hover position (not owner's Y)
                        double yDiff = targetYLevel - petLoc.getY(); // positive = need to go up, negative = need to go down
                    ...
                    Vector toOwner = new Vector(
                            ownerLoc.getX() - petLoc.getX(),
                            0,
                            ownerLoc.getZ() - petLoc.getZ()
                    );
                    Vector horizontalVel = new Vector(vel.getX(), 0, vel.getZ());
                    ...
                    // Base threshold: STEER_BASE_THRESHOLD (60 degrees), at high speed: STEER_MAX_THRESHOLD (45 degrees)
                    ...
                    if (horizontalSpeed > STEER_MIN_SPEED && toOwnerDist > STEER_MIN_DIST) {
                        Vector toOwnerNormalized = toOwner.clone().normalize();
                        ...
                        if (alignment < steerThreshold) {
                            ...
                            double newVelX = horizontalVel.getX() * keepFactor + toOwnerNormalized.getX() * horizontalSpeed * steerFactor;
                            ...
                            vel = newVel;
                            speed = vel.length();
                            horizontalVel = new Vector(vel.getX(), 0, vel.getZ());
    ...
    private void applyWalkSpeed(double distance) {
        float baseWalkSpeed = owner.getWalkSpeed();
        ...
        if (distance < CRUISE_RANGE) {
            ...
            if (isSwimmingPet()) {
                ...
                if (owner.hasPotionEffect(PotionEffectType.DOLPHINS_GRACE)) {
    ...
    public @NotNull GoalKey<Mob> getKey() {
        return PetGoalKey.FOLLOW_OWNER;
    }
